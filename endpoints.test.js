...
const {getWholeRecipe} = require("./api/recipes/recipeModel")
/**
[GET] /api/recipes
[GET] /api/recipes/:id
[POST] /api/recipes
...
describe('server.js', () => {
  ...
  describe('[POST] /api/recipes', () => {
    test('[6] creates a new recipe in the database', async () => {
      ...
      const recordCount = await db("recipes")
        ...
        .join("ingredients_steps","steps.step_id","ingredients_steps.step_id")
        .join("ingredients","ingredients.ingredient_id","ingredients_steps.ingredient_id")
        .where({recipe_name:'foo',ingredient_name:"olive oil"});
      expect(recordCount).toHaveLength(1);
      const updatedRecipes = await db('recipes');
      expect(updatedRecipes).toHaveLength(recipes.length + 1);
      const res = await request(server).get(`/api/recipes/${recipes.length + 1}`);
    ...
    test('[9] responds with a 400 and proper error if name or steps are undefined', async () => {
      const invalid1 = {}
      const invalid2 = { recipe_name: "foo" }
    ...
    test('[10] responds with a 400 and proper error if name is not a string', async () => {
      const invalid = { recipe_name: 123, steps: [{
        step_number:1,
        ingredients:[],
        instructions:"put them together and then cook them"
      }]}
    ...
    //   const invalid1 = { recipe_name: "fo", steps: [{
    //     step_number:1,
    //     ingredients:[],
    //     instructions:"put them together and then cook them"
    //   }] }
    //   const invalid2 = { recipe_name: "  fo   ", steps: [{
    //     step_number:1,
    //     ingredients:[],
    //     instructions:"put them together and then cook them"
    //   }] }
    //   const invalid3 = { recipe_name: "12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901", steps: [{
    //     step_number:1,
    //     ingredients:[],
    //     instructions:"put them together and then cook them"
    //   }] }
    ...
    //   expect(res.status).toBe(400)
    // }, 750)
    test('[12] responds with a 400 and proper error if steps is not an array', async () => {
      const invalid1 = { recipe_name: "foo", steps: NaN }
      const invalid2 = { recipe_name: "foo", steps: [] }
      let res = await request(server).post('/api/recipes').send(invalid1)
      expect(res.body.message).toMatch(/invalid/i)
      expect(res.status).toBe(400)
      // res = await request(server).post('/api/recipes').send(invalid2)
    ...
    test('[14] responds with a 400 and proper error if name already exists in the db', async () => {
      let res = await request(server).post('/api/recipes').send((await request(server).get("/api/recipes/1")).body);
  ...
  describe('[DELETE] /api/recipes/:id', () => {
    test('[23] can delete the correct recipe', async () => {
      ...
      const recipe = await db('recipes').where('recipe_id', 1).first()
  ...
  describe("[GET] /api/recipes/:recipe_id/ingredients",()=>{
    test("[26] can obtain the ingredients", async ()=>{
      ...
      let response = await request(server).post('/api/recipes').send({ 
        recipe_name: 'foo', 
        steps: [
          {
            step_number:1,
      ...
      const recipe_id = response.body.recipe_id;
      response = await request(server).get(`/api/recipes/${recipe_id}/ingredients`)
